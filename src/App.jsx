...
function App() {
  ...
  const filteredData = data.filter(x => {
    ...
    return filter.length === foundFilteredCount;
  });
  ...
  return (
    <>
      ...
      <div className="activeFilters">
        {filter.map(x => (
          <div className="filterTag" key={x}>
            <span>{x}</span>
            <button onClick={() => removeFilter(x)} className="removeButton">×</button>
          </div>
        ))}
        {filter.length > 0 && (
          <button onClick={clearFilters} className="clearAllButton">Clear All</button>
        )}
      </div>
      <div className="jobItems">
        {filteredData.map(x => <JobItem key={x.id} {...x} />)}
      </div>
    </>
  );
}
...
function FilterPanel({ filters, addFilter, removeFilter }) {
  const allTags = [];
    data.forEach(x => {
      x.tags.forEach(tag => {
    ...
    <div className="filterPanel">
      <div className="filterTags">
        {allTags.map(tag => (
          <button
            key={tag}
            className={filters.includes(tag) ? 'filterButton active' : 'filterButton'}
            onClick={() => {
